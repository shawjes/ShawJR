...
`_ai_p4c_soma_anno` <-`_ai_p4c_soma_genesymbol_uniprot_aptamer` %>%
...
# Creating annotation using bitr and fromType GeneSymbol gives the same amount of chromosome info as doing the same with fromType UniProt.
```

```{r}
setwd(dir.aidata)
fwrite(ai_p4c_soma_anno, "ai_p4c_soma_anno_fromGeneSymbol.csv.gz")
```
