...
ardata.split %>% rbindlist() %>% dplyr::select(T21, Karyotype) %>% unique()
...
```{r}
t_p4c_soma_lm_singlecluster_d21only_vif <- lm_vif %>% rbindlist(fill = TRUE) %>%
  dplyr::rename(VIF = `X[[i]]`,
                Cluster = id);
t_p4c_soma_lm_singlecluster_d21only_tidy <- lm_tidy %>% rbindlist();
t_p4c_soma_lm_singlecluster_d21only_glance <- lm_glance %>% rbindlist();
t_p4c_soma_lm_singlecluster_d21only_augment <- lm_augment %>% rbindlist()

t_p4c_soma_lm_singlecluster_d21only_vif %>%
  group_by(model_formula, Cluster) %>%
  summarise(maxVIF = max(VIF)) %>%
  ungroup() %>%
  arrange(desc(maxVIF));
t_p4c_soma_lm_singlecluster_d21only_tidy;
t_p4c_soma_lm_singlecluster_d21only_glance;
t_p4c_soma_lm_singlecluster_d21only_augment
```

```{r}
setwd(dir.ddata)
fwrite(t_p4c_soma_lm_singlecluster_d21only_vif, "t_p4f_soma_lm_singlecluster_d21only_vif.csv.gz")
fwrite(t_p4c_soma_lm_singlecluster_d21only_tidy, "t_p4f_soma_lm_singlecluster_d21only_tidy.csv.gz")
fwrite(t_p4c_soma_lm_singlecluster_d21only_augment, "t_p4f_soma_lm_singlecluster_d21only_augment.csv.gz")
fwrite(t_p4c_soma_lm_singlecluster_d21only_glance, "t_p4f_soma_lm_singlecluster_d21only_glance.csv.gz")
```