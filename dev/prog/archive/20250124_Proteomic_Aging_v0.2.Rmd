...
fit.Zlog10Abundance_modelA <- ar_p4c_soma %>%
  mutate(Uniprot_Aptamer_Chr_Cluster = paste0(Uniprot, "|", Aptamer, " (Chr", chromosome, ", ", Cluster, ")")) %>%
  select(Uniprot_Aptamer_Chr_Cluster, everything()) %>%
...
tidy.Zlog10Abundance_modelA01 <- tidy.Zlog10Abundance_modelA %>%
...
```{r}
tidy.Zlog10Abundance_modelA01 <- tidy.Zlog10Abundance_modelA %>%
  rename(Aptamer_Chr_Cluster = Aptamer_Cluster) %>%
  separate(Aptamer_Chr_Cluster, sep = "[(]", extra = "merge", remove = FALSE,
           into = c("a", "b")) %>%
  select(-c(a)) %>%
  ...
  mutate(cat_Zlog10_FC = case_when(Zlog10_FC < 0 ~ "-",
                                   Zlog10_FC > 0 ~ "+",
                                   .default = NA))
tidy.Zlog10Abundance_modelA02 <- tidy.Zlog10Abundance_modelA01 %>%
  mutate(cat_Zlog10_FC = case_when(Zlog10_FC < 0 ~ "-",
                                   Zlog10_FC > 0 ~ "+",
                                   .default = NA));


tidy.Zlog10Abundance_modelB01 <- tidy.Zlog10Abundance_modelB %>%
  rename(Aptamer_Chr_Cluster = Aptamer_Cluster) %>%
  separate(Aptamer_Chr_Cluster, sep = "[(]", extra = "merge", remove = FALSE,
           into = c("a", "b")) %>%
  select(-c(a)) %>%
  ...
  mutate(Sig10_PadjBH = case_when(Padj_BH<0.10 ~ 1, .default = 0),
         Sig05_PadjBH = case_when(Padj_BH<0.05 ~ 1, .default = 0)) %>%
  mutate(cat_Zlog10_FC = case_when(Zlog10_FC < 0 ~ "Down in T21",
                                   Zlog10_FC > 0 ~ "Up in T21",
                                   .default = NA))
tidy.Zlog10Abundance_modelB02 <- tidy.Zlog10Abundance_modelB01 %>%
  mutate(cat_Zlog10_FC = case_when(Zlog10_FC < 0 ~ "-",
                                   Zlog10_FC > 0 ~ "+",
                                   .default = NA));

tidy.Zlog10Abundance_modelA02
tidy.Zlog10Abundance_modelB02
```
```{r}
setwd("/Users/jessica/Dropbox/EspinosaGroup/ANALYSIS/2025/dev/output")
fwrite(tidy.Zlog10Abundance_modelA02, "tidy_Zlog10Abundance_modelA02.csv")

setwd("/Users/jessica/Dropbox/EspinosaGroup/ANALYSIS/2025/dev/output")
fwrite(tidy.Zlog10Abundance_modelB02, "tidy_Zlog10Abundance_modelB02.csv")
```

```{r}
rm(tidy.Zlog10Abundance_modelA01, tidy.Zlog10Abundance_modelA,
   tidy.Zlog10Abundance_modelB01, tidy.Zlog10Abundance_modelB);
gc()
```
...
```{r}
ggData_chr_distribution_SigPadjBH10.modelA02 <- tidy.Zlog10Abundance_modelA02 %>%
  arrange(P) %>%
  select(-c(std.error, statistic)) %>%
  select(Aptamer_Chr_Cluster, everything()) %>%
...
ggbar_modelA_T21sigPadjBH10_chrDist <- ggData_chr_distribution_SigPadjBH10.modelA02 %>%
  filter(term == "T21") %>%
...
setwd("/Users/jessica/Dropbox/EspinosaGroup/ANALYSIS/2025/dev/output")
ggsave(ggbar_modelA_T21sigPadjBH10_chrDist,
         filename = "20250124_ggbar_modelA_T21sigPadjBH10_chrDist_v0.2.png",
         width = 5, height = 5, units = "in")
```
