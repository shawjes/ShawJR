...
```{r}
setwd(dir.aidata)
ai_p4c_soma_genesymbol_uniprot_aptamer <- fread("ai_p4c_soma_genesymbol_uniprot_aptamer.csv.gz")
ai_p4c_soma_bitr_kegg_anno <- fread("ai_p4c_soma_bitr_kegg_anno.csv.gz"); # Prepared using bitr_kegg() in script 'ai_p4c_soma_bitr_kegg_anno.Rmd'.
ai_p4c_soma_bitr_anno <- fread("ai_p4c_soma_bitr_anno.csv.gz"); # Prepared using bitr() in script 'ai_p4c_soma_bitr_anno.Rmd'.

ai_p4c_soma_genesymbol_uniprot_aptamer
ai_p4c_soma_bitr_kegg_anno
ai_p4c_soma_bitr_anno

ai_p4c_soma_genesymbol_uniprot_aptamer_anno <- ai_p4c_soma_genesymbol_uniprot_aptamer %>%
  left_join(ai_p4c_soma_bitr_anno %>%
              group_by(SYMBOL, UNIPROT) %>%
              summarise(across(everything(), ~paste0(unique(.), collapse = "|"))) %>%
              ungroup(),
            by = c("UniProt" = "UNIPROT", "GeneSymbol" = "SYMBOL")) %>%
  left_join(ai_p4c_soma_bitr_kegg_anno %>%
              group_by(uniprot) %>%
              summarise(across(everything(), ~paste0(unique(.), collapse = "|"))) %>%
              ungroup(), by = c("UniProt" = "uniprot"));
ai_p4c_soma_genesymbol_uniprot_aptamer_anno

setwd(dir.aidata)
fwrite(ai_p4c_soma_genesymbol_uniprot_aptamer_anno, "ai_p4c_soma_genesymbol_uniprot_aptamer_anno.csv.gz")
```
...
for ( i in 1:length(bitr_totypes) ) {
  `_ai_p4c_soma_bitr_anno`[[i]] <- try(bitr((in_p4c$in_donovan2024_p4c_soma %>%
                                            ...
                                            OrgDb = org.Hs.eg.db));
  names(`_ai_p4c_soma_bitr_anno`) <- try(paste0("ai_p4c_soma_bitr_",
                                                bitr_totypes[[i]]))
}

ai_p4c_soma_bitr_anno <- `_ai_p4c_soma_bitr_anno` %>%
  reshape::merge_recurse(by = "SYMBOL");
#ai_bitr_anno
