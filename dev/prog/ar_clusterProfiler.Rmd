...
```{r}
# setwd(dir.ddata)
# fn.gene_list <- list("l_sigt21byage_cluster_A.txt",
#                    "l_sigt21byage_cluster_B.txt",                     
#                    "l_sigt21byage_cluster_C.txt",
#                    "l_sigt21byage_cluster_D.txt",
#                    "l_sigt21byage_cluster_E.txt",
#                    "l_sigt21byage_cluster_F.txt",
#                    "l_sigt21byage_cluster_G.txt",
#                    "l_sigt21byage_cluster_H.txt",
#                    "l_sigt21byage_cluster_I.txt",
#                    "l_sigt21byage_cluster_J.txt",
#                    "l_sigt21byage_cluster_K.txt",
#                    "l_sigt21byage_cluster_L.txt",
#                    "l_sigt21byage_cluster_M.txt",
#                    "l_sigt21byage_cluster_N.txt")
# setwd(dir.ddata)
# gene_lists <- fn.gene_list %>%
#   lapply(fread, header = FALSE)
# names(gene_lists) <- c("A", "B", "C", "D", "E",
#                        "F", "G", "H", "I", "J",
#                        "K", "L", "M", "N")
# gene_lists

# gene_lists <- ar_sigt21byage_d21t21clustered %>%
...
background_gene_lists %>% rbindlist(idcol = "id")
...
```{r}
enricher(gene = (gene_lists%>% rbindlist())$GeneSymbol,
...
ar_sigt21byage_d21t21clustered %>%
  dplyr::select(T21byAge_SuperCluster, Aptamer, `Chromosome(s)`, Chromosome) %>%
  unique() %>%
  group_by(T21byAge_SuperCluster, `Chromosome(s)`, Chromosome) %>%
  summarise(N_Aptamers = n()) %>%
  ungroup() %>%
  arrange(T21byAge_SuperCluster, `Chromosome(s)`) %>%
  split(., .$T21byAge_SuperCluster)
...
#### Code for original results sent to Kate Waugh:
```{r}
# https://carpentries-incubator.github.io/bioc-rnaseq/instructor/06-gene-set-analysis.html#choose-a-proper-universe

# c1 <- list()	
# c2_canonicalpathways <- list()				
# c2_kegg_medicus <- list()	
# c2_reactome <- list()	
# c3_mir <- list()	
# c3_tft <- list()	
# c4_canceratlas <- list()			
# c4_cancermodues <- list()	
# c5_biopathway <- list()	
# c5_cellularcomponent <- list()	
# c5_go <- list()	
# c5_hpo <- list()	
# c5_molecularfunction <- list()					
# c6_oncogenicsig <- list()	
# c7_immunesig <- list()	
# c8 <- list()	
# h <- list()	
...
#   names(c1)[[i]] <- paste("c1_", names(gene_lists)[[i]], sep = "|");
# 
#   # c2_canonicalpathways
#   c2_canonicalpathways[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#             TERM2GENE = bg_genes.c2_canonicalpathways,
#             pvalueCutoff = 1.0,
#              qvalueCutoff = 1,
#             pAdjustMethod = "BH"))@result);
#   names(c2_canonicalpathways)[[i]] <- paste("c2_canonicalpathways_",
#                                              names(gene_lists)[[i]], sep = "|");
#   # c2_kegg_medicus
#   c2_kegg_medicus[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c2_kegg_medicus,
#             pvalueCutoff = 1.0,
#              qvalueCutoff = 1,
#               pAdjustMethod = "BH"))@result);
#   names(c2_kegg_medicus)[[i]] <- paste("c2_kegg_medicus_", names(gene_lists)[[i]], sep = "|");
#   # c2_reactome
#   c2_reactome[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c2_reactome,
#             pvalueCutoff = 1.0,
#              qvalueCutoff = 1,
#               pAdjustMethod = "BH"))@result);
#   names(c2_reactome)[[i]] <- paste("c2_reactome_", names(gene_lists)[[i]], sep = "|");
#   # gsea_c3_mir               
#   c3_mir[[i]] <- try( (enricher(gene = gene_lists[[i]]$V1,
#               TERM2GENE = bg_genes.c3_mir,
#             pvalueCutoff = 1.0,
#              qvalueCutoff = 1,
#               pAdjustMethod = "BH"))@result);
#   names(c3_mir)[[i]] <- paste("c3_mir", names(gene_lists)[[i]], sep = "|");
#   # c3_tft 
#   c3_tft[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c3_tft,
#             pvalueCutoff = 1.0,
#             qvalueCutoff = 1,
#                pAdjustMethod = "BH"))@result);
#   names(c3_tft)[[i]] <- paste("c3_tft_", names(gene_lists)[[i]], sep = "|");
#   # c4_canceratlas       
#   c4_canceratlas[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c4_canceratlas,
#             pvalueCutoff = 1.0,
#             qvalueCutoff = 1,
#                pAdjustMethod = "BH"))@result);
#   names(c4_canceratlas)[[i]] <- paste("c4_canceratlas",
#                                         names(gene_lists)[[i]],
#                                         sep = "|");
#   # c4_cancermodues
#   c4_cancermodues[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c4_cancermodues,
#             pvalueCutoff = 1.0,
#             qvalueCutoff = 1,
#                pAdjustMethod = "BH"))@result);
#   names(c4_cancermodues)[[i]] <- paste("c4_cancermodues",
#                                         names(gene_lists)[[i]],
#                                         sep = "|");
#   # c5_biopathway				
#   c5_biopathway[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c5_biopathway,
#             pvalueCutoff = 1.0,
#             qvalueCutoff = 1,
#                pAdjustMethod = "BH"))@result);
#   names(c5_biopathway)[[i]] <- paste("c5_biopathway",
#                                         names(gene_lists)[[i]],
#                                         sep = "|");
#   # c5_cellularcomponent
#   c5_cellularcomponent[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c5_cellularcomponent,
#             pvalueCutoff = 1.0,
#             qvalueCutoff = 1,
#                pAdjustMethod = "BH"))@result);
#   names(c5_cellularcomponent)[[i]] <- paste("c5_cellularcomponent",
#                                         names(gene_lists)[[i]],
#                                         sep = "|");
#   # c5_go				
#   c5_go[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
...
#                                         names(gene_lists)[[i]],
#                                         sep = "|");
#   # c5_molecularfunction				
#   c5_molecularfunction[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
#               TERM2GENE = bg_genes.c5_molecularfunction,
#             pvalueCutoff = 1.0,
#             qvalueCutoff = 1,
#                pAdjustMethod = "BH"))@result);
#   names(c5_molecularfunction)[[i]] <- paste("c5_molecularfunction",
#                                         names(gene_lists)[[i]],
#                                         sep = "|");
#   # c6_oncogenicsig				
#   c6_oncogenicsig[[i]] <- try( (enricher(gene = gene_lists[[i]]$GeneSymbol,
...
```{r}
#n_gsea_background_databases <- 17
#gsea_significance_threshold_per_database <- 0.10/n_gsea_background_databases
```

#### t_sigt21byage_cluster_gene_position
```{r}
t_sigt21byage_cluster_gene_position <- c1 %>%
...
setwd(dir.ddata)
fwrite(t_sigt21byage_cluster_gene_position, "t_sigt21byage_cluster_gene_position.csv")
```

#### t_sigt21byage_cluster_chr21_genes_positions
```{r}
t_sigt21byage_cluster_chr21_genes_positions <- t_sigt21byage_cluster_gene_position %>%
  ...
  left_join( (t_sigt21byage_cluster_gene_position %>%
                filter(Chr21 == 1) %>%
...
setwd(dir.ddata)
fwrite(t_sigt21byage_cluster_chr21_genes_positions, "t_sigt21byage_cluster_chr21_genes_positions.csv")

t_sigt21byage_cluster_chr21_genes_positions
```

#### t_sigt21byage_cluster_chrY_genes_positions
```{r}
t_sigt21byage_cluster_chrY_genes_positions <- t_sigt21byage_cluster_gene_position %>%
  mutate(ChrY = case_when(grepl("chrY", genomic_position)==TRUE ~ 1, .default = 0)) %>%
  group_by(T21byAge_Cluster, ChrY) %>%
  summarise(N_Cluster_Aptamers_ChrY = n()) %>%
  ungroup() %>%
  mutate(ChrY = case_when(ChrY==1 ~ "Aptamers encoded by ChrY (N)",
                           ChrY==0 ~ "Aptamers encoded by other chromosome (N)",
                           .default = NA)) %>%
  dplyr::select(T21byAge_Cluster, ChrY, N_Cluster_Aptamers_ChrY) %>%
  spread(key = ChrY, value = N_Cluster_Aptamers_ChrY, fill = 0) %>%
  mutate(`Aptamers Total (N)` = `Aptamers encoded by ChrY (N)` + `Aptamers encoded by other chromosome (N)`) %>%
  mutate(`Proportion of Aptamers encoded by ChrY` = round(`Aptamers encoded by ChrY (N)`/`Aptamers Total (N)`, digits = 4)) %>%
  dplyr::select(-c(`Aptamers encoded by other chromosome (N)`)) %>%
  arrange(desc(`Aptamers encoded by ChrY (N)`)) %>%
  left_join( (t_sigt21byage_cluster_gene_position %>%
                filter(grepl("chrY", genomic_position)==TRUE)),
             by = "T21byAge_Cluster" ) %>%
  filter(!is.na(geneID)) %>%
  dplyr::rename(ChrY_encoded_geneID = geneID,
                ChrY_encoded_geneID_position = genomic_position)

setwd(dir.ddata)
fwrite(t_sigt21byage_cluster_chrY_genes_positions, "t_sigt21byage_cluster_chrY_genes_positions.csv")

t_sigt21byage_cluster_chrY_genes_positions
```

```{r}
t_sigt21byage_cluster_gsea_results_all <- rbind(c1 %>% keep(., inherits, 'data.frame')  %>% rbindlist(idcol = "gsea_id", fill = TRUE),
                          c2 %>% rbindlist(idcol = "gsea_id", fill = TRUE),
...
setwd(dir.ddata)
fwrite(t_sigt21byage_cluster_gsea_results_all, "t_sigt21byage_cluster_gsea_results_all.csv.gz")
```

#### t_sigt21byage_cluster_gsea_results_sigGSEApadjBH
```{r}
t_sigt21byage_cluster_gsea_results_sigGSEApadjBH <- t_sigt21byage_cluster_gsea_results_all %>%
  arrange(pvalue) %>%
  filter(GSEA_PadjBH<0.05) %>%
  arrange(desc(Count));

setwd(dir.ddata)
fwrite(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH, "t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.csv.gz");
```

#### t_sigt21byage_cluster_gsea_results_sigGSEApadjBH
```{r}
t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.split <- t_sigt21byage_cluster_gsea_results_sigGSEApadjBH %>%
  arrange(GSEA_background_genes) %>%
  group_by(GSEA_background_genes) %>%
  arrange(desc(Count)) %>%
  ungroup() %>%
  split(., .$T21byAge_Cluster);
names(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.split)[[2]] <- "Sig. T21-by-Age"
names(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.split)[[3]] <- "Non-sig. T21-by-Age"
names(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.split)

rm(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH); gc();
setwd(dir.ddata)
openxlsx::write.xlsx(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.split,
                     "t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.xlsx")
...
```{r}
setwd(dir.ddata)
t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.splitByGSEAbg <- t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.split %>%
  rbindlist() %>%
  split(., .$GSEA_background_genes)

t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.splitByGSEAbg
setwd(dir.ddata)
openxlsx::write.xlsx(t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.splitByGSEAbg,
                     "t_sigt21byage_cluster_gsea_results_sigGSEApadjBH.splitByGSEAbg.xlsx")
```
