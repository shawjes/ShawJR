...
```{r}
ar_p4c_daDSsumZ_rna_metab_cytof <- rbind( (ar_d21t21clustered_daDScluster_meanZ %>%
                                              mutate(Omics_type = "Proteomics (SomaLogic)",
                                                     Analyte_type = "Protein abundance",
                                                     Analyte = paste0(DA_DS_Cluster,
                                                                      " (N=", N_Cluster_Aptamers,
                                                                      " Aptamers)"),
                                                     Units = "mean(Zlog10_relative_abundance|DA-DS Cluster)",
                                                     Value = MEAN_Zlog10_relative_abundance) %>%
                                              select(LabID, Karyotype, Sex, Age,
                                                     Omics_type, Analyte_type, Analyte, Value, Units)),
                                           ar_p4c_rna,
                                           ar_p4c_metab,
                                           ar_p4c_cytof ) %>%
  full_join( (ar_d21t21clustered_daDScluster_meanZ %>%
                select(LabID, T21, Female, Karyotype_Sex) %>%
                unique()),
             by = "LabID")

head(ar_p4c_daDSsumZ_rna_metab_cytof)
```
```{r}
setwd(dir.ardata)
fwrite(ar_p4c_daDSsumZ_rna_metab_cytof, "ar_p4c_daDSsumZ_rna_metab_cytof.csv.gz")
```
...
```{r}
# setwd(dir.indata)
# in_cytof <- fread("in_donovan2024_p4c_cytof_cluster_pct.csv.gz")
# in_rna <- fread("in_donovan2024_p4c_rna_fpkm.csv.gz")
# in_metab <- fread("in_donovan2024_p4c_metab.csv.gz")
# 
# in_cytof %>% head(n=10)
# in_rna %>% head(n=10)
# in_metab %>% head(n=10)
...
ai_rna <- in_rna %>%
  ...
  unique()
ai_rna01 <- ai_rna %>%
  left_join(ar_d21t21clustered_daDScluster_meanZ,
            by = "LabID") %>%
  na.omit();

summary(ai_rna$Value)

rna_d21_mean_sd <- ai_rna %>%
  filter(Karyotype == "Control") %>%
  group_by(Analyte) %>%
  summarise(d21_mean_log10FPKM = mean(log10(Value + 0.5), na.rm=TRUE),
            d21_sd_log10FPKM = sd(log10(Value), na.rm=TRUE)) %>%
  ungroup();

ar_rna <- ai_rna %>%
  left_join(rna_d21_mean_sd, by = "Analyte") %>%
  group_by(LabID, Analyte) %>%
  mutate(Zlog10_FPKM = (log10(Value + 1e-05) - d21_mean_log10FPKM)/d21_sd_log10FPKM) %>%
  ungroup() %>%
  ...
  select(LabID, Karyotype, Sex, Age, T21, Female, Karyotype_Sex,
         Omics_type, Analyte_type, Analyte, Value, Units) 

ar_rna %>% head(n=100)

setwd(dir.ardata)
fwrite(ar_rna, "ar_p4c_rna.csv.gz")
...
ai_cytof <- in_cytof %>%
  ...
  left_join(ar_sigt21byage_d21t21clustered %>%
              select(LabID, Karyotype, Sex, Age, T21, Female, Karyotype, Karyotype_Sex) %>% 
              unique(),
            by = "LabID") %>%
  na.omit();
ar_cytof <- ai_cytof %>%
  ...
  summarise(d21_mean_log10percentOfCD45posCD66low = mean(log10(Value), na.rm=TRUE),
            d21_sd_log10percentOfCD45posCD66low = sd(log10(Value), na.rm=TRUE)) %>%
  ungroup() %>%
  full_join(ai_cytof, by = "CyTOF_cell_population") %>%
  group_by(LabID, CyTOF_cell_population) %>%
  mutate(Zlog10_percentOfCD45posCD66low = (log10(Value) - d21_mean_log10percentOfCD45posCD66low)/d21_sd_log10percentOfCD45posCD66low) %>%
  ungroup() %>%
  select(LabID, Karyotype, Sex, Age, T21, Female, Karyotype_Sex,
         CyTOF_cell_population, Zlog10_percentOfCD45posCD66low) %>%
  mutate(Units = "Zlog10_percentOfCD45posCD66low") %>%
  dplyr::rename(Analyte = CyTOF_cell_population,
                Value = Zlog10_percentOfCD45posCD66low) %>%
  mutate(Omics_type = "CyTOF",
...
ar_cytof_metab_rna <- rbind(ar_cytof, ar_metab, ar_rna)

ar_cytof_metab_rna$Analyte %>% unique() %>% length() #[1] 194

ar_cytof_metab_rna.split <- ar_cytof_metab_rna %>%
  split(., .$Analyte)

rm(ar_cytof, ar_metab, ar_rna,
   in_cytof,
   ai_cytof, ai_metab);
gc()

ar_cytof_metab_rna %>% nrow()
ar_cytof_metab_rna %>% na.omit() %>% nrow()
```

```{r}
lm_tidy <- ar_cytof_metab_rna.split %>%
    #head(n=3) %>%
