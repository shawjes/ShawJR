...
#### f_dendrogram_sigt21byage_d21t21_clustered
```{r}
setwd(dir.output)
pdf("f_dendrogram_sigt21byage_d21t21_clustered.pdf")
k_hc1_newclusters.D21s <- 14
plot(hc1_newclusters.D21s, cex = 0.1);
rect.hclust(hc1_newclusters.D21s,
            k = k_hc1_newclusters.D21s, 
...
#### f_dendrogram_sigt21byage_d21t21clustered_dendextend
```{r}
library(dendextend)
library(colorspace)
set.seed(1234)
tmp <- loess_prediction.D21.matrix
rownames(tmp) <- NULL
f_dendrogram_sigt21byage_d21t21clustered_dendextend <- (hclust(dist(tmp, method = "euclidean"), method = "complete")) %>%
                                                              as.dendrogram() %>%
                                                              color_branches(k = 14,
                                                                             groupLabels = LETTERS[1:14]) %>%
  assign_values_to_leaves_nodePar(nodePar = "lab.cex",
                                  value = 0.25) %>%
  hang.dendrogram(0.5) %>%
  color_labels(col="white")

setwd(dir.output)
pdf("f_dendrogram_sigt21byage_d21t21clustered_dendextend.pdf") #, paper = "USr")
plot(f_dendrogram_sigt21byage_d21t21clustered_dendextend)
dev.off()
```

#### Use the cuttree function to assign aptamers to k=14 T21-by-Age Clusters ('ar_sigt21byage_d21t21clustered')
```{r echo=T, results='hide'}
k_hc1_newclusters.D21s <- 14
set.seed(1234)
tmp1.ar_sigt21byage_d21t21clustered <- cutree(hc1_newclusters.D21s, k = k_hc1_newclusters.D21s) %>%
    as.data.frame() %>%
...
colnames(tmp1.ar_sigt21byage_d21t21clustered)

tmp2.ar_sigt21byage_d21t21clustered <- tmp1.ar_sigt21byage_d21t21clustered %>%
  dplyr::select(T21byAge_Cluster, UniProt_Aptamer_Chr_Cluster) %>%
  unique() %>%
  group_by(T21byAge_Cluster) %>%
  summarise(N_Cluster_Aptamers = n()) %>%
  ungroup() %>%
  full_join(tmp1.ar_sigt21byage_d21t21clustered, by = "T21byAge_Cluster") 

#left_join( fread(paste0(dir.ardata,
  #                        "/ar_sigt21byage_loess_predicted_values.csv.gz")),
  #           by = "UniProt_Aptamer_Chr_Cluster" )

head(tmp2.ar_sigt21byage_d21t21clustered)
```

#### ar_sigt21byage_d21t21clustered
```{r}
tmp3.ar_sigt21byage_d21t21clustered <- tmp2.ar_sigt21byage_d21t21clustered %>%
  # Create T21-by-Age super-Clusters:
  # - SuperCluster FNC:
  # F, N, C are all monotonic decreasing trajectories with D21 value ranges of -1.0 to 2.0;
  # F and N values among T21 remains lower than D21 values throughout the lifespan.
  # - SuperCluster KLI:
  # L, K  and I are all increasing curves with  ranges in D21 ~-1.25 to 1, and T21 remains higher than D21 throughout the lifespan.
  # - SuperCluster HM:
  # H and M both have D21 y-value ranges ~-0.5 to 1.0. 
  # HM_H, HM_M
  mutate(T21byAge_SuperCluster = case_when(T21byAge_Cluster %in% c("F", "N", "C") ~ "FNC",
                                           T21byAge_Cluster %in% c("L", "K", "I") ~ "KLI",
                                           T21byAge_Cluster %in% c("M", "H") ~ "MH",
                                           .default = T21byAge_Cluster)#,
         #T21byAge_Cluster = case_when(T21byAge_Cluster %in% c("F", "N", "C") ~ paste0("FNC_", T21byAge_Cluster),
        #                                   T21byAge_Cluster %in% c("L", "K", "I") ~ paste0("KLI_", T21byAge_Cluster),
        #                                   T21byAge_Cluster %in% c("M", "H") ~ paste0("MH_", T21byAge_Cluster),
        #                              .default = T21byAge_Cluster)
        )

ar_sigt21byage_d21t21clustered <- tmp3.ar_sigt21byage_d21t21clustered %>%
  dplyr::select(T21byAge_Cluster, T21byAge_SuperCluster, N_Cluster_Aptamers) %>%
  unique() %>%
  group_by(T21byAge_SuperCluster) %>%
  summarise(N_SuperCluster_Aptamers = sum(N_Cluster_Aptamers)) %>%
  ungroup() %>%
  full_join(tmp3.ar_sigt21byage_d21t21clustered, by = c("T21byAge_SuperCluster")) %>%
  dplyr::select(T21byAge_Cluster, T21byAge_SuperCluster, N_Cluster_Aptamers, N_SuperCluster_Aptamers, everything())
...
setwd(dir.ardata)
fwrite(ar_sigt21byage_d21t21clustered, "ar_sigt21byage_d21t21clustered.csv.gz")
```

#### f_bar_k_vs_naptamers_per_cluster
#### gg_bar_k_vs_naptamers_per_cluster
#### Use the cuttree function to assign aptamers to k=1:14 T21-by-Age Clusters ('ar_sigt21byage_d21t21clustered')
```{r echo=T, results='hide'}
k_hc1_newclusters.D21s <- c(seq(1:14))
set.seed(1234)
tmp.ar_sigt21byage_d21t21clustered <- cutree(hc1_newclusters.D21s, k = k_hc1_newclusters.D21s) %>%
    as.data.frame() %>%
  `colnames<-`(paste0("k", names(.), "_T21byAge_Cluster")) %>%
    rownames_to_column("UniProt_Aptamer_Chr_Cluster") %>%
    ungroup() %>%
  left_join(ar_sigt21byage_loess_predicted_values, by = "UniProt_Aptamer_Chr_Cluster") %>%
  # gather(key = "key", value = "value", starts_with("k'")) %>%
...
f_bar_k_vs_naptamers_per_cluster <- tmp.ar_sigt21byage_d21t21clustered %>%
  dplyr::select(UniProt_Aptamer_Chr_Cluster, contains("T21byAge_Cluster")) %>%
  unique() %>%
  gather(key = "k", value = "T21byAge_Cluster", starts_with("k")) %>%
  unique() %>%
  mutate(k = gsub("_T21byAge_Cluster", "", k),
         k = gsub("k", "", k),
         k = as.numeric(k)) %>%
  group_by(k, T21byAge_Cluster) %>%
  summarise(N_Aptamers = n()) %>%
  ungroup() %>%
  mutate(k = factor(k, levels = c(seq(1:20)))) %>%
  group_by(k) %>%
  arrange(T21byAge_Cluster) %>%
  ungroup() %>%
  arrange(k, T21byAge_Cluster);

gg_bar_k_vs_naptamers_per_cluster <- f_bar_k_vs_naptamers_per_cluster %>%
  ggplot(aes(x = reorder(k, -N_Aptamers),
             y = N_Aptamers, color = T21byAge_Cluster, fill = T21byAge_Cluster,
             width = 0.6)) +
...
setwd(dir.ddata)
fwrite(f_bar_k_vs_naptamers_per_cluster, "f_bar_k_vs_naptamers_per_cluster.csv")

setwd(dir.output)
ggsave(gg_bar_k_vs_naptamers_per_cluster,
     filename = "f_bar_k_vs_naptamers_per_cluster.png",
...
ar_sigt21byage_d21t21clustered %>%
  ...
  geom_smooth(method = "loess",
              se = TRUE) +
  theme(aspect.ratio = 1.0) +
  facet_wrap(~T21byAge_SuperCluster)
```

# ARCHIVE BELOW FOR NOW (2/25/2025 ~11AM MT)

#### Evaluate gap statistics
```{r}
# https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/hierarchical-cluster-analysis/#:~:text=A%20Simple%20Example
# set.seed(1234)  
# gap_stat <- cluster::clusGap(loess_prediction.D21.matrix,
#                              FUN = hclust,
#                              K.max = 14,
#                              B = 50) # number of Monte Carlo (“bootstrap”) samples
...
```{r}
ar_sigt21byage_aptamer_clusters <- ar_sigt21byage_d21t21clustered %>%
  dplyr::select(UniProt_Aptamer_Chr_Cluster, T21byAge_Cluster) %>%
  unique()
...
```{r}
# ar_sigt21byage_d21t21clustered.split <- ar_sigt21byage_d21t21clustered %>% split(., .$T21byAge_Cluster) 
# 
# gg_trajectory_bykt_sigt21byage_cluster <- list()
# gg_trajectory_byktsex_sigt21byage_cluster <- list()
# for ( i in 1:length(ar_sigt21byage_d21t21clustered.split) ) {
#   gg_trajectory_bykt_sigt21byage_cluster[[i]] <- ar_sigt21byage_d21t21clustered.split[[i]] %>%
...
#     ggtitle(paste0("T21-by-Age Cluster ", names(ar_sigt21byage_d21t21clustered.split)[[i]])) +
#     theme(aspect.ratio = 1.0,
#           legend.position = "bottom",
#           legend.title = element_blank());
#   save_as <- paste0("f_trajectory_bykt_sigt21byage_cluster_",
#                     names(ar_sigt21byage_d21t21clustered.split)[[i]]);
#   setwd(dir.output)
#   ggsave(gg_trajectory_bykt_sigt21byage_cluster[[i]],
#        filename = paste0(save_as, ".png"),
#        width = 5, height = 5, units = "in")
# 
#   gg_trajectory_byktsex_sigt21byage_cluster[[i]] <- ar_sigt21byage_d21t21clustered.split[[i]] %>%
#     ggplot(aes(x = Age, y = Zlog10_relative_abundance, color = Karyotype_Sex, fill = Karyotype_Sex)) +
#     geom_smooth(method = "loess", linewidth = 0.75, se = TRUE) +
...
```{r}
# gg_sigt21byage_d21t21clustered_bykaryotypesex <- ar_sigt21byage_d21t21clustered %>%
...
#### Use the cuttree function to assign aptamers to k=12 T21-by-Age Clusters ('ar_sigt21byage_d21t21clustered')
```{r echo=T, results='hide'}
k_hc1_newclusters.D21s <- 12
set.seed(1234)
...
k_hc1_newclusters.D21s <- 12
set.seed(1234)
ar_sigt21byage_d21t21clustered <- cutree(hc1_newclusters.D21s, k = k_hc1_newclusters.D21s) %>%
    as.data.frame() %>%
    `colnames<-`("T21byAge_Cluster") %>%
    rownames_to_column("UniProt_Aptamer_Chr_Cluster") %>%
    mutate(k = k_hc1_newclusters.D21s) %>%
