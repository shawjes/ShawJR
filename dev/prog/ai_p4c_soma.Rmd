...
ai_in_donovan2024_p4c_soma <- in_donovan2024_p4c_soma %>%
  ...
  full_join( (in_donovan2024_p4c_soma %>%
                ...
                sd_log10_relative_abundance_d21 = sd(log10_relative_abundance)) %>%
                ungroup()),
             by = "Aptamer" ) %>%
      full_join( (in_donovan2024_p4c_soma %>%
                    dplyr::select(-c(Data_contact, Date_exported, Script, Units)) %>%
                  ...
                  summarise(mean_log10_relative_abundance_t21 = mean(log10_relative_abundance),
                            sd_log10_relative_abundance_t21 = sd(log10_relative_abundance)) %>%
                  ungroup()),
            by = "Aptamer" ) %>%
    group_by(Aptamer) %>%
...
setwd(dir.aidata)
fwrite(ai_in_donovan2024_p4c_soma, "ai_p4c_soma.csv.gz")
```
